...
class CNN_biLSTM(nn.Module):
    def __init__(self, device=0):
        ...
        self.lstm_1 = nn.LSTM(input_size=16, hidden_size=32, num_layers=1, bias=True, bidirectional=True, batch_first=True)
        self.w_h_h = nn.Linear(32*2, 32, bias=False)
    ...
    def forward(self, batch):
        ...
        hidden_state = state[0].transpose(0,1).contiguous()
        hidden_state = hidden_state.view(hidden_state.size(0), 2*32)
        hidden_state = self.w_h_h(hidden_state)
